...
- Options:
  ...
  - For local development runs, provision the environment from `pyproject.toml` / `uv.lock` first (for example, `uv sync --project .`) so the runtime uses the locked dependencies.
