...
class AiGuardrailsProvider(ToolProvider):
    def _validate_credentials(self, credentials: dict[str, Any]) -> None:
        ...
        if not region:
            raise ToolProviderCredentialValidationError("地域不能为空。")

        try:
            AiGuardrailsTool.from_credentials(credentials).invoke(tool_parameters={"modalType": "text", "content": "这是一条测试数据", "type": "input"})
