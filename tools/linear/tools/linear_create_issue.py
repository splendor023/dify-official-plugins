...
class LinearCreateIssueTool(Tool):
    ...
    def _invoke(
        ...
        try:
            ...
            assignee_id = (
                tool_parameters.get("assigneeId", "").strip()
                if tool_parameters.get("assigneeId")
                else None
            )
            priority_param = tool_parameters.get(
                "priority"
            )  # Renamed to avoid conflict
            labels = tool_parameters.get(
                "labels", []
            )  # Assuming labels are passed as a list of IDs
            status_id = tool_parameters.get(
                "statusId"
            )  # Get statusId if provided (might need adjustment in YAML too)
            ...
            if priority_param is not None:
                try:
                    ...
                    if 0 <= priority_value <= 4:
                        ...
                        yield self.create_text_message(
                            f"Warning: Invalid priority value '{priority_param}'. Must be between 0 and 4. Skipping priority."
                        )
            ...
            if labels:
                if isinstance(labels, list) and all(
                    ...
                    yield self.create_text_message(
                        f"Warning: Invalid format for labels. Expected a list of strings (label IDs). Skipping labels."
                    )
            ...
            result = linear_client.query_graphql(
                query=graphql_mutation, variables={"input": mutation_input}
            )
            ...
            if (
                ...
                and "data" in result
                and result["data"]
                and "issueCreate" in result["data"]
            ):
                ...
                if issue_result and issue_result.get("success"):
                    ...
                    if issue:
                        ...
                        yield self.create_text_message(
                            "Issue created successfully, but no issue details were returned."
                        )
                    return
                else:
            ...
            yield self.create_text_message(
                "Error: Failed to create issue - unexpected API response."
            )
