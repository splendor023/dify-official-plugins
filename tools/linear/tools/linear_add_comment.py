...
class LinearAddCommentTool(Tool):
    ...
    def _invoke(
        ...
        try:
            ...
            body = tool_parameters.get(
                "body", ""
            ).strip()  # Don't strip by default? Let user control formatting?
            ...
            if len(body) > 50000:
                yield self.create_text_message(
                    "Error: comment body cannot exceed 50000 characters"
                )
                return
            ...
            if (
                ...
                and "data" in result
                and result["data"]
                and "commentCreate" in result["data"]
            ):
                ...
                if comment_result and comment_result.get("success"):
                    ...
                    if comment:
                        yield self.create_text_message(
                            f"Comment added successfully to issue {issue_id}. Comment ID: {comment.get('id')}"
                        )
                        ...
                        yield self.create_text_message(
                            "Comment added successfully, but no comment details were returned."
                        )
                    return
                else:
            ...
            yield self.create_text_message(
                "Error: Failed to add comment - unexpected API response."
            )
        ...
        # Updated exception handling
            ...
            yield self.create_text_message(
                f"Error adding comment: Issue with ID '{issue_id}' might not exist or cannot be commented on. Details: {str(e)}"
            )
