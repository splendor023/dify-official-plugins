...
class LinearGetUserIssuesTool(Tool):
    ...
    def _invoke(
    ...
    ) -> Generator[ToolInvokeMessage, None, None]:  # Renamed params to tool_parameters
        ...
        try:
            ...
            team_id = (
                tool_parameters.get("teamId", "").strip()
                if tool_parameters.get("teamId")
                else None
            )
            state = (
                tool_parameters.get("state", "").strip()
                if tool_parameters.get("state")
                else None
            )
            limit = min(
                int(tool_parameters.get("limit", 10)), 50
            ...
            if not uuid_pattern.match(user_id):
                yield self.create_text_message(
                    f"Error: Invalid userId format. Please provide a valid UUID (e.g., '123e4567-e89b-12d3-a456-426614174000'). Provided: {user_id}"
                )
                return

            # Use GraphQL variables to prevent injection attacks
            # Build filter object with required and optional conditions
            ...
            # Build GraphQL query
            graphql_query = """
            query GetUserIssues($filter: IssueFilter!, $limit: Int!) {
              issues(
                filter: $filter,
                first: $limit
              ) {
            ...
            if result and "data" in result and "issues" in result.get("data", {}):
                ...
                if not issues:
                    yield self.create_text_message(
                        f"No issues found for user with ID {user_id}"
                    )
                    return
            ...
            yield self.create_text_message(
                "Error: Failed to retrieve user issues - unknown error"
            )
