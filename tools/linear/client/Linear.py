...
class Linear:
    ...
    def set_api_key(self, LINEAR_API_KEY):
        if LINEAR_API_KEY and len(LINEAR_API_KEY) >= 4:
            self._log("API key has been set.")
        else:
            self._log("Warning: API key is None or too short (< 4 characters).")
    ...
    def query_graphql(self, query, variables=None):
        ...
        try:
            ...
            response = requests.post(
                self.graphql_url, json=request_data, headers=self.headers
            )
            ...
            if "errors" in response_data:
                self._log(
                    f"GraphQL errors in response: {json.dumps(response_data['errors'])}"
                )
    ...
    def query_basic_resource(self, resource="", first=50, after=None, variables=None):
        ...
        query_variables.update({"first": first, "after": after})
    ...
    def create_issue(
        ...
        issue_id = (
            create_response["data"]["issueCreate"]["issue"]["id"] if success else None
        )
        self._log(
            f"Issue creation {'successful' if success else 'failed'}{f', ID: {issue_id}' if issue_id else ''}"
        )
    ...
    def search_issues(
        self, query=None, filter=None, first=50, after=None, order_by="updatedAt"
    ):
        """
        ...
        """
        self._log(
            f"Searching issues. query: '{query}', orderBy: {order_by}, first: {first}"
        )
        if filter:
            self._log(f"Search filter: {json.dumps(filter)}")

        variables = {"first": first, "after": after, "orderBy": order_by}
        ...
        self._log(
            f"Search found {issues_count} issues{', with more results available' if has_next else ''}"
        )
    ...
    def get_issue(self, issue_id):
        ...
        if issue:
            self._log(
                f"Retrieved issue: {issue.get('identifier', '')} - {issue.get('title', '')}"
            )
        else:
    ...
    def get_viewer(self):
        ...
        self._log(
            f"Retrieved viewer info: {viewer.get('name')} ({viewer.get('email')})"
        )
    ...
    def get_team_issues(self, team_id, first=50, after=None, filter=None):
        ...
        variables = {"id": team_id, "first": first, "after": after}
        ...
        if team:
            ...
            self._log(
                f"Retrieved {issues_count} issues for team: {team.get('name', team_id)}"
            )
